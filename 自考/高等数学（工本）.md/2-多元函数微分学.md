...
#### 全微分
- 可导 <=> 可微。  偏导连续 <=> 可微
- 若 $z=f(x,y)$，则全微分 $d_z = \frac{\partial z}{\partial x}d_x + \frac{\partial z}{\partial y}d_y$ ，通常求在某一点处的值，对x的偏导数和对y的偏导数可以得到， $d_x$和 $d_y$不可消除。 最后得到  $d_z_{(x_0,y_0)} =  ad_x + bd_y  $

- 若 $u=f(x,y,z)$，则全微分 $d_u = \frac{\partial u}{\partial x}d_x + \frac{\partial u}{\partial y}d_y +  \frac{\partial u}{\partial z}d_z$ 。 
  - 例：全微分 $u = x^{z \over y}$ ， 求 $d_u_{(e,1,1)}$
...
- 隐函数通常会结合全微分或者复合函数偏导数出题。 总是先求隐函数，再求对应的。
  - 例函数 $z = z(x,y) $由方程 $e^z + xyz + x + cos x = 2 $确定，求 $d_z_{(0,1)}$ 答案 $-d_x$ 
